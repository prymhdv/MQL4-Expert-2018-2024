...
#include <.RTL_Prymhdv\QuartsExperts\A_Enviroments(MQL4)\Include\Parser\jsonParser\hash.mqh>
...
public:
          ...
          static bool getString(JSONValue *val, string &out)    { if (val != NULL && val.isString())    { out = val.getString();        return true; } return false; }
          static bool getBool(JSONValue *val, bool &out)        { if (val != NULL && val.isBool())      { out = val.getBool();          return true; } return false; }
          static bool getDouble(JSONValue *val, double &out)    { if (val != NULL && val.isNumber())    { out = val.getDouble();        return true; } return false; }
          static bool getLong(JSONValue *val, long &out)        { if (val != NULL && val.isNumber())    { out = val.getLong();          return true; } return false; }
          static bool getInt(JSONValue *val, int &out)          { if (val != NULL && val.isNumber())    { out = val.getInt();           return true; } return false; } };
...
private:
          string _string;
public:
          JSONString(string s)          { setString(s); setType(JSON_STRING); }
...
private:
          bool _bool;
public:
          JSONBool(bool b)      { setBool(b); setType(JSON_BOOL); }
...
private:
          ...
          double _dbl;
public:
          JSONNumber(long l)            { _long = l; _dbl = 0; }
          JSONNumber(double d)          { _long = 0; _dbl = d; }
          long getLong()                { if (_dbl != 0)  { return (long)_dbl; }        else { return _long; }          }
          int getInt()                  { if (_dbl != 0)  { return (int)_dbl; }         else { return (int)_long; }     }
          double getDouble()            { if (_long != 0) { return (double)_long; }     else { return _dbl; }           }
          string toString()             { if (_long != 0) { return (string)_long; }     else { return (string)_dbl; }   }  //   /* Favour the long*/
};
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
public:
          ...
          ~JSONObject()         { if (_hash != NULL) delete _hash; }
...
public:
          ...
          ~JSONArray()  { for(int i = ArrayRange(_array, 0) - 1 ; i >= 0 ; i-- )        { delete _array[i]; }  }  // clean up array
          // Getters for Objects (key lookup ) --------------------------------------
          // Get child members - halt program if wront type(cast error) or doesnt exist(null pointer)
...
public:
          ...
          {         int inLen;
                    ...
                    if (_len != inLen + 1 /* nul */ )   {         setError(1, StringConcatenate("unable to create array ", inLen, " got ", _len)); }
                    else
                    {         _len --;
                              ret = parseValue();
                              if (_errCode != 0)
          ...
          {         JSONValue *ret = NULL;
                    ...
                    {         string s1 = parseString();
                              ret = (JSONValue*)new JSONString(s1);
                              //
                    }
...
//|                                                                  |
//+------------------------------------------------------------------+
#define __DEC(TYPE,CLASSNAME,OBJNAME)
class CLASSNAME: public JsonItem
{
public:
          CLASSNAME() {};
          CLASSNAME(string json)  {this.parse(typename(this), json);}
          //--TYPE get()              {return(TYPE)m_value;};
//
} ;  CLASSNAME OBJNAME;
//+------------------------------------------------------------------+
//|                                                                  |
