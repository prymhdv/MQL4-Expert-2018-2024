...
#include "Picture.mqh"
#include <.RTL_Prymhdv\QuartsExperts\A_Enviroments(MQL4)\Include\Canvas\Canvas.mqh>
#include <.RTL_Prymhdv\QuartsExperts\A_Enviroments(MQL4)\Include\Tools\DateTime.mqh>
//+------------------------------------------------------------------+
//| Enumerations                                                     |
//+------------------------------------------------------------------+
//--- date modes
...
//--- header height + 7 rows
   int h=(CONTROLSg_BUTTON_SIZE+4*CONTROLS_BORDER_WIDTH)+7*(2*CONTROLS_FONT_SIZE);
...
//---
   ...
   x=Width()/2;
   y=CONTROLSg_BUTTON_SIZE/2+2*CONTROLS_BORDER_WIDTH;
   switch(m_mode)
     {
      //--- within the month
      case DATE_MODE_MON:
               ...
               if(tmp_date.mon==m_value.mon)
                 {
                  ...
                  m_canvas.TextOut(x,y,text,COLOR2RGB(CONTROLSg_BUTTON_COLOR_BORDER),text_al);
               tmp_date.DayInc();
              }
           }
         break;
