...
//+------------------------------------------------------------------+
class CFile : public CObject_MQL4
{
protected:
          int               m_handle;             // handle of file     If a file has been opened successfully, the function returns the file handle
          string            m_name;               // name of opened file
          int               m_flags;              // flags of opened file

public:
          CFile(void): m_handle(INVALID_HANDLE),  m_name(""),  m_flags(FILE_ANSI) {};
          ~CFile(void) { if(m_handle != INVALID_HANDLE) Close(); }
          ...
          void              Flush(void)                                                 { if(m_handle != INVALID_HANDLE) FileFlush(m_handle); } //Flush data from the file buffer of input-output to disk
          bool              IsEnding(void);
          bool              IsLineEnding(void);
          //--- general methods for working with files
          void              Delete(const string file_name, const int common_flag = 0);
          bool              IsExist(const string file_name, const int common_flag = 0)  { return(FileIsExist(file_name, common_flag)); } ////| Check if file exists
...
{         //--- check handle
          ...
          {         //--- store options of the opened file
                    m_flags |= open_flags; m_name = file_name; //
          }
          //--- result
          return(m_handle);//
}
