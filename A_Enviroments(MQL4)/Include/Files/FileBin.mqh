...
public:
          ...
          int               Open(const string file_name, const int open_flags) { return(CFile::Open(file_name, open_flags | FILE_BIN)); } // Opening a binary file   //!! drive can use :: un static metod call
          ...
          bool              WriteObject(CObject_MQL4 *object);
          template<typename T>
          ...
          template<typename T>
          uint              ReadStruct(T &data,int sizebyte=-1);
          bool              ReadObject(CObject_MQL4 *object);
          template<typename T>
...
{         //--- check handle
          if(m_handle != INVALID_HANDLE) return(FileWriteStruct(m_handle, data));
          //--- failure
          return(0); }
//+------------------------------------------------------------------+
//| Write data of an instance of the CObject_MQL4 class                   |
//+------------------------------------------------------------------+
bool CFileBin::WriteObject(CObject_MQL4 *object)
{         //--- check handle & object
          if(m_handle != INVALID_HANDLE)
...
//+------------------------------------------------------------------+
template<typename T>
uint CFileBin::ReadStruct(T &data,int sizebyte=-1)
{         //--- check handle
          if(m_handle != INVALID_HANDLE)return(FileReadStruct(m_handle, data,sizebyte));
          //--- failure
          return(0); }
//+------------------------------------------------------------------+
//| Read data of an instance of the CObject_MQL4 class                    |
//+------------------------------------------------------------------+
bool CFileBin::ReadObject(CObject_MQL4 *object)
{         //--- check handle & object
          if(m_handle != INVALID_HANDLE)
