...
public:
          ...
          bool              AddArray(const CArrayInt_MQL4 *src);
          bool              Insert(const int element, const int pos);
          bool              InsertArray(const int &src[], const int pos);
          bool              InsertArray(const CArrayInt_MQL4 *src, const int pos);
          bool              AssignArray(const int &src[]);
          bool              AssignArray(const CArrayInt_MQL4 *src);
          ...
          bool              CompareArray(const CArrayInt_MQL4 *array) const;
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CArrayInt_MQL4::CArrayInt_MQL4(void)
...
//+------------------------------------------------------------------+
CArrayInt_MQL4::~CArrayInt_MQL4(void)
{         if(m_data_max != 0) Shutdown(); ArrayFree(m_data); }
//+------------------------------------------------------------------+
//| Moving the memory within a single array                          |
//+------------------------------------------------------------------+
int CArrayInt_MQL4::MemMove(const int dest, const int src, const int count)
{         int i;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Reserve(const int size)
{         int new_size;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Resize(const int size)
{         int new_size;
          //--- check
          if(size < 0)
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Shutdown(void)
{         //--- check
          if(m_data_max == 0)
                    return(true);
          //--- clean
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Add(const int element)
{         //--- check/reserve elements of array
          if(!Reserve(1))return(false);
          //--- add
          m_data[m_data_total++] = element; m_sort_mode = -1;  ///zero++
          //--- successful
          return(true); }
//+------------------------------------------------------------------+
//| Adding an element to the end of the array from another array     |
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::AddArray(const int &src[])
...
//| Adding an element to the end of the array from another array     |
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::AddArray(const CArrayInt_MQL4 *src)
{         int num;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Insert(const int element, const int pos)
{         //--- check/reserve elements of array
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::InsertArray(const int &src[], const int pos)
{         int num = ArraySize(src);
          //--- check/reserve elements of array
          if(!Reserve(num))return(false);
          //--- insert
...
//| Inserting elements in the specified position                     |
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::InsertArray(const CArrayInt_MQL4 *src, const int pos)
{         int num;
          //--- check
          if(!CheckPointer(src))return(false);
          //--- check/reserve elements of array
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::AssignArray(const int &src[])
{         int num = ArraySize(src);
          //--- check/reserve elements of array
          Clear();
          if(m_data_max < num)
          ...
          //--- copy array
          for(int i = 0; i < num; i++) {         m_data[i] = src[i]; m_data_total++; }
          m_sort_mode = -1;
          //--- successful
          return(true); }
//+------------------------------------------------------------------+
//| Assignment (copying) of another array                            |
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::AssignArray(const CArrayInt_MQL4 *src)
{         int num;
          //--- check
          if(!CheckPointer(src))return(false);
          //--- check/reserve elements of array
          ...
          Clear();
          if(m_data_max < num) {         if(!Reserve(num))return(false); }
          else Resize(num);
          //--- copy array
          for(int i = 0; i < num; i++) {         m_data[i] = src.m_data[i]; m_data_total++; }
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::At(const int index) const
{         //--- check
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Update(const int index, const int element)
{         //--- check
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Shift(const int index, const int shift)
{         int tmp_int;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Delete(const int index)
{         //--- check
          if(index < 0 || index >= m_data_total)return(false);
          //--- delete
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::DeleteRange(int from, int to)
{         //--- check
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::CompareArray(const int &array[]) const
{         //--- compare
          if(m_data_total != ArraySize(array))return(false);
          for(int i = 0; i < m_data_total; i++) {if(m_data[i] != array[i]) {return(false);}}
          //--- equal
          return(true); }
//+------------------------------------------------------------------+
//| Equality comparison of two arrays                                |
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::CompareArray(const CArrayInt_MQL4 *array) const
{         //--- check
          if(!CheckPointer(array))return(false);
          //--- compare
          if(m_data_total != array.m_data_total)return(false);
          for(int i = 0; i < m_data_total; i++) { if(m_data[i] != array.m_data[i]) {    return(false);}}
          //--- equal
          return(true); }
//+------------------------------------------------------------------+
//| Method QuickSort                                                 |
//+------------------------------------------------------------------+
void CArrayInt_MQL4::QuickSort(int beg, int end, const int mode)
{         int  i, j;
          int  p_int, t_int;
          //--- check
          if(beg < 0 || end < 0)
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::InsertSort(const int element)
{         int pos;
          //--- check
          ...
          if(m_data_total == 0) {         m_data[m_data_total++] = element; return(true); }
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::SearchLinear(const int element) const
{         //--- check
          if(m_data_total == 0)
                    return(-1);
          //---
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::QuickSearch(const int element) const
{         int  i, j, m = -1;
          int  t_int;
          //--- search
          i = 0;
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::Search(const int element) const
{         int pos;
          //--- check
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::SearchGreat(const int element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::SearchLess(const int element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::SearchGreatOrEqual(const int element) const
{         //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
          //--- search
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::SearchLessOrEqual(const int element) const
{         //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
          //--- search
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::SearchFirst(const int element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayInt_MQL4::SearchLast(const int element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Save(const int file_handle)
{         int i = 0;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayInt_MQL4::Load(const int file_handle)
{         int i = 0, num;
          //--- check
