...
//+------------------------------------------------------------------+
//| Class CArrayObj_MQL4.                                                 |
//| Puprose: Class of dynamic array of pointers to instances         |
//|          of the CObject_MQL4 class and its derivatives.               |
//|          Derives from class CArray_MQL4.                              |
//+------------------------------------------------------------------+
class CArrayObj_MQL4 : public CArray_MQL4
{
protected:
          CObject_MQL4     *m_data[];           // data array
...
public:
          ...
          bool              Add(CObject_MQL4 *element);
          bool              AddArray(const CArrayObj_MQL4 *src);
          bool              Insert(CObject_MQL4 *element, const int pos);
          bool              InsertArray(const CArrayObj_MQL4 *src, const int pos);
          bool              AssignArray(const CArrayObj_MQL4 *src);
          //--- method of access to thre array
          CObject_MQL4      *At(const int index) const;
          CObject_MQL4      *operator[](const int index) const;
          //--- methods of changing
          bool              Update(const int index, CObject_MQL4 *element);
          bool              Shift(const int index, const int shift);
          //--- methods of deleting
          CObject_MQL4          *Detach(const int index);
          bool              Delete(const int index);
          bool              DeleteRange(int from, int to);
          void              Clear(void);
          //--- method for comparing arrays
          bool              CompareArray(const CArrayObj_MQL4 *array) const;
          //--- methods for working with the sorted array
          bool              InsertSort(CObject_MQL4 *element);
          int               Search(const CObject_MQL4 *element) const;
          int               SearchGreat(const CObject_MQL4 *element) const;
          int               SearchLess(const CObject_MQL4 *element) const;
          int               SearchGreatOrEqual(const CObject_MQL4 *element) const;
          int               SearchLessOrEqual(const CObject_MQL4 *element) const;
          int               SearchFirst(const CObject_MQL4 *element) const;
          int               SearchLast(const CObject_MQL4 *element) const;

protected:
          void              QuickSort(int beg, int end, const int mode);
          int               QuickSearch(const CObject_MQL4 *element) const;
          int               MemMove(const int dest, const int src, const int count); };
//+------------------------------------------------------------------+
//| Constructor                                                      |
//+------------------------------------------------------------------+
CArrayObj_MQL4::CArrayObj_MQL4(void) : m_free_mode(true)
{         //--- initialize protected data
          m_data_max = ArraySize(m_data); }
//+------------------------------------------------------------------+
//| Destructor                                                       |
//+------------------------------------------------------------------+
CArrayObj_MQL4::~CArrayObj_MQL4(void)
{         if(m_data_max != 0)
                    Shutdown(); }
//+------------------------------------------------------------------+
//| Moving the memory within a single array                          |
//+------------------------------------------------------------------+
int CArrayObj_MQL4::MemMove(const int dest, const int src, const int count)
{         int i;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Reserve(const int size)
{         int new_size;
          //--- check
          if(size <= 0)
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Resize(const int size)
{         int new_size;
          //--- check
          if(size < 0)
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Shutdown(void)
{         //--- check
          if(m_data_max == 0)
                    return(true);
          //--- clean
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Add(CObject_MQL4 *element)
{         //--- check
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::AddArray(const CArrayObj_MQL4 *src)
{         int num;
          //--- check
          if(!CheckPointer(src))
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Insert(CObject_MQL4 *element, const int pos)
{         //--- check
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::InsertArray(const CArrayObj_MQL4 *src, const int pos)
{         int num;
          //--- check
          if(!CheckPointer(src))
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::AssignArray(const CArrayObj_MQL4 *src)
{         int num;
          //--- check
          if(!CheckPointer(src))
                    return(false);
...
//+------------------------------------------------------------------+
CObject_MQL4 *CArrayObj_MQL4::At(const int index) const
{         //--- check
...
//|                                                                  |
//+------------------------------------------------------------------+
CObject_MQL4 *CArrayObj_MQL4::operator[](const int index) const
{
          {         //--- check
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Update(const int index, CObject_MQL4 *element)
{         //--- check
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Shift(const int index, const int shift)
{         CObject_MQL4 *tmp_node;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Delete(const int index)
{         //--- check
...
//+------------------------------------------------------------------+
CObject_MQL4 *CArrayObj_MQL4::Detach(const int index)
{         CObject_MQL4 *result;
          //--- check
          if(index >= m_data_total)
                    return(NULL);
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::DeleteRange(int from, int to)
{         //--- check
          if(from < 0 || to < 0)
                    return(false);
...
//+------------------------------------------------------------------+
void CArrayObj_MQL4::Clear(void)
{         //--- "physical" removal of the object (if necessary and possible)
          if(m_free_mode)
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::CompareArray(const CArrayObj_MQL4 *array) const
{         //--- check
...
//| Method QuickSort                                                 |
//+------------------------------------------------------------------+
void CArrayObj_MQL4::QuickSort(int beg, int end, const int mode)
{         int      i, j;
          CObject_MQL4 *p_node;
          CObject_MQL4 *t_node;
          //--- sort
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::InsertSort(CObject_MQL4 *element)
{         int pos;
          //--- check
...
//+------------------------------------------------------------------+
int CArrayObj_MQL4::QuickSearch(const CObject_MQL4 *element) const
{         int      i, j, m = -1;
          CObject_MQL4 *t_node;
          //--- search
          i = 0;
...
//+------------------------------------------------------------------+
int CArrayObj_MQL4::Search(const CObject_MQL4 *element) const
{         int pos;
          //--- check
...
//+------------------------------------------------------------------+
int CArrayObj_MQL4::SearchGreat(const CObject_MQL4 *element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !CheckPointer(element) || m_sort_mode == -1)
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayObj_MQL4::SearchLess(const CObject_MQL4 *element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !CheckPointer(element) || m_sort_mode == -1)
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayObj_MQL4::SearchGreatOrEqual(const CObject_MQL4 *element) const
{         //--- check
          if(m_data_total == 0 || !CheckPointer(element) || m_sort_mode == -1)
                    return(-1);
          //--- search
...
//+------------------------------------------------------------------+
int CArrayObj_MQL4::SearchLessOrEqual(const CObject_MQL4 *element) const
{         //--- check
          if(m_data_total == 0 || !CheckPointer(element) || m_sort_mode == -1)
                    return(-1);
          //--- search
...
//+------------------------------------------------------------------+
int CArrayObj_MQL4::SearchFirst(const CObject_MQL4 *element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !CheckPointer(element) || m_sort_mode == -1)
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayObj_MQL4::SearchLast(const CObject_MQL4 *element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !CheckPointer(element) || m_sort_mode == -1)
                    return(-1);
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Save(const int file_handle)
{         int i = 0;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayObj_MQL4::Load(const int file_handle)
{         int i = 0, num;
          //--- check
