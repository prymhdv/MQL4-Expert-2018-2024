...
//|          CObject_MQL4 instances and its dervivatives                  |
//|          Derives from class CObject_MQL4.                             |
//+------------------------------------------------------------------+
class CList_MQL4 : public CObject_MQL4
  {
protected:
   CObject_MQL4          *m_first_node;       // pointer to the first element of the list
   CObject_MQL4          *m_last_node;        // pointer to the last element of the list
   CObject_MQL4          *m_curr_node;        // pointer to the current element of the list
...
public:
   ...
   virtual CObject_MQL4  *CreateElement(void) { return(NULL); }
   //--- methods of filling the list
   int               Add(CObject_MQL4 *new_node);
   int               Insert(CObject_MQL4 *new_node,int index);
   //--- methods for navigating
   int               IndexOf(CObject_MQL4 *node);
   CObject_MQL4          *GetNodeAtIndex(int index);
   CObject_MQL4          *GetFirstNode(void);
   CObject_MQL4          *GetPrevNode(void);
   CObject_MQL4          *GetCurrentNode(void);
   CObject_MQL4          *GetNextNode(void);
   CObject_MQL4          *GetLastNode(void);
   //--- methods for deleting
   CObject_MQL4          *DetachCurrent(void);
   bool              DeleteCurrent(void);
   bool              Delete(int index);
   void              Clear(void);
   //--- method for comparing lists
   bool              CompareList(CList_MQL4 *List);
   //--- methods for changing
   void              Sort(int mode);
   bool              MoveToIndex(int index);
   bool              Exchange(CObject_MQL4 *node1,CObject_MQL4 *node2);
   //--- method for searching
   CObject_MQL4          *Search(CObject_MQL4 *element);
protected:
   void              QuickSort(int beg,int end,int mode);
   CObject_MQL4          *QuickSearch(CObject_MQL4 *element);
  };
//+------------------------------------------------------------------+
//| Constructor                                                      |
//+------------------------------------------------------------------+
CList_MQL4::CList_MQL4(void) : m_first_node(NULL),
                     m_last_node(NULL),
                     m_curr_node(NULL),
                     m_curr_idx(-1),
                     m_data_total(0),
...
//| Method QuickSort                                                 |
//+------------------------------------------------------------------+
void CList_MQL4::QuickSort(int beg,int end,int mode)
  {
   int      i,j,k;
   CObject_MQL4 *i_ptr,*j_ptr,*k_ptr;
//---
...
//+------------------------------------------------------------------+
int CList_MQL4::IndexOf(CObject_MQL4 *node)
  {
//--- check
...
//+------------------------------------------------------------------+
int CList_MQL4::Add(CObject_MQL4 *new_node)
  {
//--- check
...
//+------------------------------------------------------------------+
int CList_MQL4::Insert(CObject_MQL4 *new_node,int index)
  {
...
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::GetNodeAtIndex(int index)
  {
   int      i;
   bool     revers;
   CObject_MQL4 *result;
...
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::GetFirstNode(void)
  {
//--- check
...
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::GetPrevNode(void)
  {
//--- check
...
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::GetCurrentNode(void)
  {
   return(m_curr_node);
  }
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::GetNextNode(void)
  {
//--- check
...
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::GetLastNode(void)
  {
//--- check
...
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::DetachCurrent(void)
  {
   CObject_MQL4 *tmp_node,*result=NULL;
//--- check
...
//+------------------------------------------------------------------+
bool CList_MQL4::DeleteCurrent(void)
  {
   CObject_MQL4 *result=DetachCurrent();
//--- check
   if(result==NULL)
      return(false);
//--- complete the processing of element removed from the list
...
//+------------------------------------------------------------------+
bool CList_MQL4::Delete(int index)
  {
   if(GetNodeAtIndex(index)==NULL)
      return(false);
//--- result
...
//+------------------------------------------------------------------+
bool CList_MQL4::CompareList(CList_MQL4 *List)
  {
   CObject_MQL4 *node,*lnode;
//--- check
...
//+------------------------------------------------------------------+
bool CList_MQL4::MoveToIndex(int index)
  {
//--- check
...
//| current one                                                      |
//+------------------------------------------------------------------+
bool CList_MQL4::Exchange(CObject_MQL4 *node1,CObject_MQL4 *node2)
  {
   CObject_MQL4 *tmp_node,*node;
//--- check
...
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::QuickSearch(CObject_MQL4 *element)
  {
   int      i,j,m;
   CObject_MQL4 *t_node=NULL;
//--- check
   if(m_data_total==0)
      return(NULL);
...
//| Search position of an element in a sorted list                   |
//+------------------------------------------------------------------+
CObject_MQL4 *CList_MQL4::Search(CObject_MQL4 *element)
  {
   CObject_MQL4 *result;
//--- check
...
//+------------------------------------------------------------------+
bool CList_MQL4::Save(const int file_handle)
  {
   CObject_MQL4 *node;
   bool     result=true;
//--- check
...
//+------------------------------------------------------------------+
bool CList_MQL4::Load(const int file_handle)
  {
   uint     i,num;
   CObject_MQL4 *node;
   bool     result=true;
