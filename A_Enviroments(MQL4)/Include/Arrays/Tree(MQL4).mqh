...
//|                                              http://www.mql4.com |
//+------------------------------------------------------------------+
#include  "TreeNode(MQL4).mqh"
//+------------------------------------------------------------------+
//| Class CTree_MQL4.                                                     |
//| Purpose: Building a binary tree of instances of CTreeNode_MQL4 class  |
//|          and its derviatives.                                    |
//|          Derives from class CTreeNode_MQL4.                           |
//+------------------------------------------------------------------+
class CTree_MQL4 : public CTreeNode_MQL4
{
protected:
          CTreeNode_MQL4        *m_root_node;        // root node of the tree

public:
          CTree_MQL4(void);
          ~CTree_MQL4(void);
          //--- methods of access to protected data
          CTreeNode_MQL4        *Root(void) const { return(m_root_node); }
          //--- method of identifying the object
          virtual int       Type() const { return(0x9999); }
          //--- method of filling the tree
          CTreeNode_MQL4        *Insert(CTreeNode_MQL4 *new_node);
          //--- methods of removing tree nodes
          bool              Detach(CTreeNode_MQL4 *node);
          bool              Delete(CTreeNode_MQL4 *node);
          void              Clear(void);
          //--- method of searching data in the tree
          CTreeNode_MQL4        *Find(const CTreeNode_MQL4 *node);
          //--- method to create elements in the tree
          virtual CTreeNode_MQL4 *CreateElement() { return(NULL); }
          //--- methods for working with files
          virtual bool      Save(const int file_handle);
          virtual bool      Load(const int file_handle);

protected:
          void              Balance(CTreeNode_MQL4 *node);//
};
//+------------------------------------------------------------------+
//| Constructor                                                      |
//+------------------------------------------------------------------+
CTree_MQL4::CTree_MQL4(void) : m_root_node(NULL)
{}
//+------------------------------------------------------------------+
//| Destructor                                                       |
//+------------------------------------------------------------------+
CTree_MQL4::~CTree_MQL4(void)
{         Clear(); }
//+------------------------------------------------------------------+
//| Method of adding a node to the tree                              | return root
//+------------------------------------------------------------------+
CTreeNode_MQL4 *CTree_MQL4::Insert(CTreeNode_MQL4 *new_node)
{         CTreeNode_MQL4 *p_node;
          CTreeNode_MQL4 *result = m_root_node;
          //--- check
...
//+------------------------------------------------------------------+
bool CTree_MQL4::Delete(CTreeNode_MQL4 *node)
{         //--- check
...
//+------------------------------------------------------------------+
bool CTree_MQL4::Detach(CTreeNode_MQL4 *node)
{         CTreeNode_MQL4 *curr_node, *tmp_node;
          CTreeNode_MQL4 *nodeA, *nodeB;
          //--- check
          curr_node = node;
...
//+------------------------------------------------------------------+
CTreeNode_MQL4 *CTree_MQL4::Find(const CTreeNode_MQL4 *node)
{         CTreeNode_MQL4 *result = m_root_node;
          //--- find
...
//+------------------------------------------------------------------+
void CTree_MQL4::Balance(CTreeNode_MQL4 *node)
{         CTreeNode_MQL4 *nodeA, *nodeB, *nodeC, *curr_node, *tmp_node;
          //---
          curr_node = node;
...
//+------------------------------------------------------------------+
bool CTree_MQL4::Save(const int file_handle)
{         //--- check
...
//+------------------------------------------------------------------+
bool CTree_MQL4::Load(const int file_handle)
{         //--- check
          if(file_handle == INVALID_HANDLE)
                    return(false);
