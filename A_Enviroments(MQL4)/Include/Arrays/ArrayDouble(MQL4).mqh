...
public:
          ...
          bool              AddArray(const CArrayDouble_MQL4 *src);
          bool              Insert(const double element, const int pos);
          bool              InsertArray(const double &src[], const int pos);
          bool              InsertArray(const CArrayDouble_MQL4 *src, const int pos);
          bool              AssignArray(const double &src[]);
          bool              AssignArray(const CArrayDouble_MQL4 *src);
          ...
          bool              CompareArray(const CArrayDouble_MQL4 *array) const;
...
//| Constructor                                                      |
//+------------------------------------------------------------------+
CArrayDouble_MQL4::CArrayDouble_MQL4(void) : m_delta(0.0)
{         m_data_max = ArraySize(m_data); }
//+------------------------------------------------------------------+
//| Destructor                                                       |
//+------------------------------------------------------------------+
CArrayDouble_MQL4::~CArrayDouble_MQL4(void)
{         if(m_data_max != 0)
                    Shutdown(); }
//+------------------------------------------------------------------+
//| Moving the memory within a single array                          |
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::MemMove(const int dest, const int src, const int count)
{         int i;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Reserve(const int size)
{         int new_size;
          //--- check
          if(size <= 0)
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Resize(const int size)
{         int new_size;
          //--- check
          if(size < 0)
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Shutdown(void)
{         //--- check
          if(m_data_max == 0)
                    return(true);
          //--- clean
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Add(const double element)
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::AddArray(const double &src[])
...
//| Adding an element to the end of the array from another array     |
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::AddArray(const CArrayDouble_MQL4 *src)
{         int num;
          //--- check
          if(!CheckPointer(src))
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Insert(const double element, const int pos)
{         //--- check/reserve elements of array
          if(pos < 0 || !Reserve(1))
                    return(false);
          //--- insert
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::InsertArray(const double &src[], const int pos)
{         int num = ArraySize(src);
          //--- check/reserve elements of array
          if(!Reserve(num))
                    return(false);
...
//| Inserting elements in the specified position                     |
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::InsertArray(const CArrayDouble_MQL4 *src, const int pos)
{         int num;
          //--- check
          if(!CheckPointer(src))
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::AssignArray(const double &src[])
{         int num = ArraySize(src);
          //--- check/reserve elements of array
          Clear();
          if(m_data_max < num)
...
//| Assignment (copying) of another array                            |
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::AssignArray(const CArrayDouble_MQL4 *src)
{         int num;
          //--- check
          if(!CheckPointer(src))
                    return(false);
...
//+------------------------------------------------------------------+
double CArrayDouble_MQL4::At(const int index) const
{         //--- check
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Update(const int index, const double element)
{         //--- check
          if(index < 0 || index >= m_data_total)
                    return(false);
          //--- update
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Shift(const int index, const int shift)
{         double tmp_double;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Delete(const int index)
{         //--- check
          if(index < 0 || index >= m_data_total)
                    return(false);
          //--- delete
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::DeleteRange(int from, int to)
{         //--- check
          if(from < 0 || to < 0)
                    return(false);
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::CompareArray(const double &array[]) const
{         //--- compare
...
//| Equality comparison of two arrays                                |
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::CompareArray(const CArrayDouble_MQL4 *array) const
{         //--- check
...
//| Method QuickSort                                                 |
//+------------------------------------------------------------------+
void CArrayDouble_MQL4::QuickSort(int beg, int end, const int mode)
{         int    i, j;
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::InsertSort(const double element)
{         int pos;
          //--- check
          if(!IsSorted())
                    return(false);
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::SearchLinear(const double element) const
{         //--- check
          if(m_data_total == 0)
                    return(-1);
          //---
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::QuickSearch(const double element) const
{         int    i, j, m = -1;
          double t_double;
          //--- search
          i = 0;
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::Search(const double element) const
{         int pos;
          //--- check
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::SearchGreat(const double element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::SearchLess(const double element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::SearchGreatOrEqual(const double element) const
{         //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
          //--- search
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::SearchLessOrEqual(const double element) const
{         //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
          //--- search
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::SearchFirst(const double element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
...
//+------------------------------------------------------------------+
int CArrayDouble_MQL4::SearchLast(const double element) const
{         int pos;
          //--- check
          if(m_data_total == 0 || !IsSorted())
                    return(-1);
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Save(const int file_handle)
{         int i = 0;
          //--- check
...
//+------------------------------------------------------------------+
bool CArrayDouble_MQL4::Load(const int file_handle)
{         int i = 0, num;
          //--- check
