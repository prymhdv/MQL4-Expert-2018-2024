...
//+------------------------------------------------------------------+
class CChartObjectText : public CChartObject_MQL4
{
public:
          ...
          //---Extr DEV Handler
          //--- Fake attribe
          void              RayRight( bool RAY_RIGHT)   {}; ///----fake From OPRYMHDV (ByPass Generic Calls))
...
//| Create object "Text"                                             |
//+------------------------------------------------------------------+
bool CChartObjectText::Create(long chart_id, const string name, const int window, const datetime time, const double price)
{         //---
...
{         if(!ObjectCreate(chart_id, name, OBJ_LABEL, window, 0, 0.0))
          ...
          Hidden(true);
//--- successful
          //--Alert(name, "  -successful-  ", __LINE__, __FUNCSIG__);
          return(true); //
}
//+------------------------------------------------------------------+
//| Get the X-distance                                               |
...
public:
          ...
          virtual bool      Save(const int file_handle);
          virtual bool      Load(const int file_handle);
          //--same create methode with Edite
          //------------------------------------------------
          //---Extr DEV
          bool              CreateCall(long ChartID_, string Name_, int SubWindow_) {return Create(  ChartID_, Name_, SubWindow_, 0, 0, 0, 0);}
          //------------------------------------------------
