...
//| ArrayXObj expands to contain new element but only in case no holes   |
//| appear in new array.                                             |
//+------------------------------------------------------------------+
template<typename T>
void ArrayInsertXObj(T &array[],int index,T &value,int extraBuffer=10)
  {
...
//+------------------------------------------------------------------+
template<typename T>
void ArrayDeleteXObj(T &array[],int index)
  {
   int size=ArraySize(array);
   if(index<0 || index>=size) return;
...
//+------------------------------------------------------------------+
template<typename T>
int ArrayDeleteAllXObj(T &array[],const T &value)
  {
...
//+------------------------------------------------------------------+
template<typename T>
int ArrayFindXObj(const T &array[],const T &value)
  {
   int s=ArraySize(array);
   int index=-1;
   for(int i=0; i<s; i++)
...
//+------------------------------------------------------------------+
template<typename T>
int BinarySearchXObj(const T &array[],T &value)
  {
   int size = ArraySize(array);
...
//+------------------------------------------------------------------+
template<typename T>
bool ArrayFindMatchXObj(const T &a[],const T &b[],T &result)
  {
...
//+------------------------------------------------------------------+
template<typename T>
int ArrayBatchRemoveXObj(T &array[],const int &removed[])
  {
   int s=ArraySize(array);
   int i=0;
   int k=0;
...
protected:
   void              clearArray();
public:
                     ArrayXObj(int extraBuffer=10):m_extraBuffer(extraBuffer) {resize(0);}
                    ~ArrayXObj() {clearArray(); ArrayFree(m_array);}
   ...
   void              insertAt(int index,T value) {ArrayInsertXObj(m_array,index,value,m_extraBuffer);}
   void              removeAt(int index) {ArrayDeleteXObj(m_array,index);}
   int               removeAll(const T value) {return ArrayDeleteAllXObj(m_array,value);}
   int               removeBatch(const int &removed[]) {return ArrayBatchRemoveXObj(m_array,removed);}
...
//+------------------------------------------------------------------+
//| call ArrayFindXObj will not compile because of template error        |
//+------------------------------------------------------------------+
template<typename T>
int ArrayXObj::index(const T &value) const
  {
   int s=ArraySize(m_array);
   int index=-1;
   for(int i=0; i<s; i++)
