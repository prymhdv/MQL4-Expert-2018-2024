...
protected:
          ...
          {         int s = ArraySize(m_array);
                    if(m_owned && s > 0)
                    {         for(int i = 0; i < s; i++)
                              {         SafeDeleteXObj(m_array[i]);}//
                    } //
          }
//------------------------------------------------------------------------
public:
          VectorXObj(uint cols, bool owned = true, int extraBuffer = 0, EqualityComparerXObj<T>*comparer = NULL): ListXObj<T>(owned, comparer), m_extraBuffer(extraBuffer)
          {         resize(cols); }
          VectorXObj(): ListXObj<T>(true, NULL), m_extraBuffer(0)
          {         resize(0); }
          ...
          ~VectorXObj() { clearArray(); ArrayFree(m_array);   }
          ...
          // Sequence interface
          void insertAt(int i, T &val)                          { ArrayInsertXObj(m_array, i, val, m_extraBuffer); }
          T removeAt(int i)                                    { T &val = m_array[i]; ArrayDeleteXObj(m_array, i); return val; }
