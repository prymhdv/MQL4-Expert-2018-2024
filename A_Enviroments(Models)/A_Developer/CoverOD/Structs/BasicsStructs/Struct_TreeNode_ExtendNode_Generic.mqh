...
//+------------------------------------------------------------------+
//| Describe CDSTreeNode_Generic_ExtendNode class derived from CDSTreeNode.         |
//+------------------------------------------------------------------+
//| Class CDSTreeNode_Generic_ExtendNode.                                 |
//| Purpose: Class of element of a binary tree.                      |
//|             Descendant of class CDSTreeNode.                  |
//+------------------------------------------------------------------+
template<typename T>
class CDSTreeNode_Generic_ExtendNode : public T //CDSTreeNode_Generic<T>
{
protected:
          //--- user's data
...
public:
          CDSTreeNode_Generic_ExtendNode();
...
protected:
          ...
          virtual T* CreateSample();//
};
//+------------------------------------------------------------------+
//| CDSTreeNode_Generic_ExtendNode class constructor.                                   |
//| INPUT:  none.                                                    |
//| OUTPUT: none.                                                    |
//| REMARK: none.                                                    |
//+------------------------------------------------------------------+
template<typename T>
void CDSTreeNode_Generic_ExtendNode::CDSTreeNode_Generic_ExtendNode()
...
//+------------------------------------------------------------------+
template<typename T>
int CDSTreeNode_Generic_ExtendNode::Compare(const CDSObject *node, int mode)
{         //--- mode parameter is ignored, because tree construction algorithm is the only one
          int res = 0;
          //--- explicit type casting
          const CDSTreeNode_Generic_ExtendNode *n = node;
...
//| INPUT:  none.                                                    |
//| OUTPUT: pointer to a new instance of CDSTreeNode_Generic_ExtendNode class.          |
//| REMARK: none.                                                    |
//+------------------------------------------------------------------+
template<typename T>
T* CDSTreeNode_Generic_ExtendNode::CreateSample()
{         CDSTreeNode_Generic_ExtendNode *result = new CDSTreeNode_Generic_ExtendNode;
          //---
          return(result); //
}
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
template<typename T>
bool CDSTreeNode_Generic_ExtendNode::Save(const int file_handle)
{         uint i = 0, len;
          //--- checks
...
//+------------------------------------------------------------------+
template<typename T>
bool CDSTreeNode_Generic_ExtendNode::Load(const int file_handle)
{         uint i = 0, len;
          //--- checks
          if(file_handle < 0) return(false);
          //--- reading
