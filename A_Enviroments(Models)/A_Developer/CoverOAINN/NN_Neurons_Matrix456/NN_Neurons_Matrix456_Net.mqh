...
#include<.RTL_Prymhdv\QuartsExperts\A_Enviroments(Models)\A_Developer\CoverOAINN\NN_Neurons_Matrix456\NN_Neurons_Matrix456_Layer.mqh>
//+------------------------------------------------------------------+
//|   Link instances to form a fully connected neural network        |
//+------------------------------------------------------------------+
class NN_Neurons_Matrix456_CNet: public CObject_MQL4
{
private:
          double                        m_error ;//every element has own error
          NN_Neurons_Matrix456_CLayer      m_layers[];
          bool                          errorFlag;
...
public:   //Create the network with the given configuration
          NN_Neurons_Matrix456_CNet(const CArrayInt_MQL4 &f_topology, const NN_Neurons_Matrix456_ActFunctions hidden = NNNM456_TanH, const NN_Neurons_Matrix456_ActFunctions output = NNNM456_TanH,
                                 double fBias = 1.0, double LearningRate = 0.15, double Momentom = 0.5, double RASF = 100);
          NN_Neurons_Matrix456_CNet()      {};
          ~NN_Neurons_Matrix456_CNet()     {}
          bool                          IsEmptyFile;
          //---
          //---
...
double NN_Neurons_Matrix456_CNet::m_recentAverageSmoothingFactor = 100.0;
//+------------------------------------------------------------------+
//|                                                                  | /*&& !IsStopped()*/
//+------------------------------------------------------------------+
NN_Neurons_Matrix456_CNet::NN_Neurons_Matrix456_CNet(const CArrayInt_MQL4 &f_topology,
                    const NN_Neurons_Matrix456_ActFunctions hidden = NNNM456_TanH, const NN_Neurons_Matrix456_ActFunctions output = NNNM456_TanH,
                    double fBias = 1.0, double LearningRate = 0.15, double Momentom = 0.5, double RASF = 100)
{         //-----set m_topology for save
          ...
          //--- create layers
          ArrayFree(m_layers);
          ArrayResize(m_layers, numLayers); //Print("NN_Neurons_Matrix456_CNet numLayers: ", numLayers,"  ArraySize(m_layers): ",ArraySize(m_layers));
          for(uint layerNum = 0; layerNum < numLayers && !IsStopped(); ++layerNum)
          {         uint numOutputs = (layerNum == f_topology.Total() - 1) ? 0 : f_topology.At(layerNum + 1);
                    //--- shorten layer pointer
                    NN_Neurons_Matrix456_CLayer *layer = GetPointer(m_layers[layerNum]);
                    ...
                    for(int neuronNum = 0; neuronNum <= f_topology.At(layerNum) && !IsStopped(); ++neuronNum)
                    {         NN_Neurons_Matrix456_CNeuron *neuron = new NN_Neurons_Matrix456_CNeuron(numOutputs, neuronNum, layer.getActivation(), LearningRate, Momentom);// make 40% more learn in midle layers!
                              if(neuron == NULL) { shutDown("Neuron creation error"); break; }
                              layer.Add(neuron);
                              if(neuronNum == f_topology.At(layerNum))neuron.setNodeVal(fBias); //??1.0
                    }//
          }//
}
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
void NN_Neurons_Matrix456_CNet::getResults(CArrayDouble_MQL4 & resultVals)const
{         if(errorFlag) return;
          resultVals.Clear();
          NN_Neurons_Matrix456_CLayer *lastLayer = GetPointer(m_layers[ArraySize(m_layers) - 1]);
          for(int n = 0; n < lastLayer.Total() - 1 && !IsStopped(); ++n)//>>-1 is bias
          {         NN_Neurons_Matrix456_CNeuron *neuron = lastLayer.At(n);
...
//+------------------------------------------------------------------+
void NN_Neurons_Matrix456_CNet::feedForward_Net(const CArrayDouble_MQL4 & inputVals, string s)
{         if(errorFlag)return;
          //---
          NN_Neurons_Matrix456_CLayer *inputLayer = GetPointer(m_layers[0]);
          if(inputVals.Total() != inputLayer.Total() - 1) {shutDown(0, s + " Invalid pointer"); return;}
          //---Set inp Neurons val from inputs val
          for(int i = 0; i < inputVals.Total() && !IsStopped(); ++i) { NN_Neurons_Matrix456_CNeuron *neuron = inputLayer.At(i); neuron.setNodeVal(inputVals.At(i)); }
          //---
          for(int layerNum = 1; layerNum < ArraySize(m_layers) && !IsStopped(); ++layerNum)
          {         NN_Neurons_Matrix456_CLayer *layer     = GetPointer(m_layers[layerNum]);
                    NN_Neurons_Matrix456_CLayer *prevLayer = GetPointer(m_layers[layerNum - 1]);
                    //---
                    for(int n = 0; n < layer.Total() - 1 && !IsStopped(); ++n)
                    {         NN_Neurons_Matrix456_CNeuron *neuron = layer.At(n); neuron.feedForward_Neuron(prevLayer);//
                    }//
          }//
}
//+------------------------------------------------------------------+
...
//+------------------------------------------------------------------+
void NN_Neurons_Matrix456_CNet::backProp_Net(const CArrayDouble_MQL4 & targetVal)
{         if(errorFlag)return;
          //---
          NN_Neurons_Matrix456_CLayer *outputLayer = GetPointer(m_layers[ArraySize(m_layers) - 1]);
          m_error = 0.0;
          for(int n = 0; n < outputLayer.Total() - 1 && !IsStopped(); ++n)
          {         const NN_Neurons_Matrix456_CNeuron *neuron = outputLayer.At(n);
          ...
          for(int n = 0; n < outputLayer.Total() - 1 && !IsStopped(); ++n)
          {         NN_Neurons_Matrix456_CNeuron *neuron = outputLayer.At(n); neuron.calcOutputGradients(targetVal.At(n)); }
          //--- make hidden neurons calculate their hidden gradients
          for(int layerNum = ArraySize(m_layers) - 2; layerNum > 0 && !IsStopped(); --layerNum)
          {         NN_Neurons_Matrix456_CLayer *hiddenLayer = GetPointer(m_layers[layerNum]);
                    NN_Neurons_Matrix456_CLayer *nextLayer = GetPointer(m_layers[layerNum + 1]);
                    //---
                    for(int n = 0; n < hiddenLayer.Total() && !IsStopped(); ++n)
                    {         NN_Neurons_Matrix456_CNeuron *neuron = hiddenLayer.At(n); neuron.calcHiddenGradients(nextLayer); } //
          }
          ...
          {         NN_Neurons_Matrix456_CLayer *layer = GetPointer(m_layers[layerNum]);
                    ...
                    //---
                    for(int n = 0; n < layer.Total() - 1 && !IsStopped(); ++n)
                    {         NN_Neurons_Matrix456_CNeuron *neuron = layer.At(n); neuron.updateInputWeights(prevLayer);}//
          }//
}
...
//+------------------------------------------------------------------+
bool NN_Neurons_Matrix456_CNet::Save(int &file_handle)
{         if(!IsStopped())
          {         //Print("*** NN_Neurons_Matrix456_CNet::Save **");
                    if(file_handle == INVALID_HANDLE) {FileClose(file_handle); Print("NN_Neurons_Matrix456_CNet::Save INVALID_HANDLE"); return false;}
                    //---
                    if(0)
                    {         uchar SartSaveFlag[];
...
//+------------------------------------------------------------------+
bool NN_Neurons_Matrix456_CNet::Load(int &file_handle, CArrayInt_MQL4 &fTopology)
{         //Print("*** NN_Neurons_Matrix456_CNet::Load **");
          if(file_handle == INVALID_HANDLE) {Print("NN_Neurons_Matrix456_CNet::Load INVALID_HANDLE"); return false;}
          if(FileSize(file_handle) == 0)    { IsEmptyFile = 0; Print("NN_Neurons_Matrix456_CNet::Load FILE_EMPTY"); return false;}
          else                              { IsEmptyFile = 1; }
          if(0)
          {         uchar ExistFlag[];
          ...
          //Print("*** NN_Neurons_Matrix456_CNet::Load >>FileReadArray**");
          ArrayFree(m_topology);
          ...
          if(1)
                    ...
                    {         //Print("*** NN_Neurons_Matrix456_CNet::Load>> for layerNumSave **", "  layerNumSave:", layerNumSave); Sleep(0);
                              uint numOutputs = (layerNumSave == numLayersSave - 1) ? 0 : m_topology[layerNumSave + 1];
                              //--- shorten layer pointer
                              NN_Neurons_Matrix456_CLayer *layer = GetPointer(m_layers[layerNumSave]);
                              //--- create neurons and give them layer activation functions
                              ...
                              {         if(m_topology[layerNumSave] != fTopology.At(layerNumSave))
                                        ...
                                        //Print("*** NN_Neurons_Matrix456_CNet::Load>> for neuronNumSave >>**", "  layerNumSave:", layerNumSave, "  neuronNumSave:", neuronNumSave); Sleep(0);
                                        NN_Neurons_Matrix456_CNeuron *neuron = new NN_Neurons_Matrix456_CNeuron(numOutputs, neuronNumSave);//at load neuron activefunc is default
                                        if(neuron == NULL) { shutDown("Neuron creation error"); break; }
          ...
          //---
          //---
          //Print("*** NN_Neurons_Matrix456_CNet::Load>> m_layers[i].Load **");
          for(int i = 0; i < ArraySize(m_layers); i++) m_layers[i].Load(file_handle);
          //---
          //Print("*** NN_Neurons_Matrix456_CNet::Load>> FileReadArray successFlag **");
          uchar successFlag[];
