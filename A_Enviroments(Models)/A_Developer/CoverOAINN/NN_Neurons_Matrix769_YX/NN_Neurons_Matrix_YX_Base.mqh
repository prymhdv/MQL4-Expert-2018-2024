...
//+------------------------------------------------------------------+
enum NN_Neurons_Matrix_YX_ActFunctions
{         NNNM_YX_ReLU,      // f(n)==a== if x < 0 : return 0  else return n
          NNNM_YX_posline,
          NNNM_YX_Purelim,   //f(n)==a==n
          NNNM_YX_Sigmoid,   // f(n)==a== return  1.0 / (1 + MathPow(2.71828, -n));   ///hardlimit ///semantic hardlimit
          NNNM_YX_SigmoidSymetric,
          NNNM_YX_SoftMax,
          NNNM_YX_TanH,      //tansigneded
          NNNM_YX_CosH,
          NNNM_YX_HyperTan,  //f(n)==a==n
          NNNM_YX_HardLimit,
          NNNM_YX_HardLimitSymetric,
          NNNM_YX_Saturate,
          NNNM_YX_SaturateSymetic,
...
class NN_Neurons_Matrix_YX_SConnection
{
public:
          NN_Neurons_Matrix_YX_SConnection(void)                { weight = randWeight();  deltaWeight = 0.0; } //weight = MathRandomNonZero();
          NN_Neurons_Matrix_YX_SConnection(bool IsFristLayer)   { if(IsFristLayer) { weight = 1; deltaWeight = 0.0;} else { weight = randWeight();  deltaWeight = 0.0;} } //weight = MathRandomNonZero();
          ~NN_Neurons_Matrix_YX_SConnection(void)    {}
