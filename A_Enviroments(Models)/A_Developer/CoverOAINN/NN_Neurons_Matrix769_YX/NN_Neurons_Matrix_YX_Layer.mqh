...
//+------------------------------------------------------------------+
#include<.RTL_Prymhdv\QuartsExperts\A_Enviroments(Models)\A_Developer\CoverOAINN\NN_Neurons_Matrix769_YX\NN_Neurons_Matrix_YX_Neuron.mqh>
//+------------------------------------------------------------------+
//|                                                                  |
//|                                                                  |
...
//+------------------------------------------------------------------+
class NN_Neurons_Matrix_YX_CLayer: public CArrayObj_MQL4 //----array of object make layer
{
public:
          uint                                  iOutputs;
          NN_Neurons_Matrix_YX_ActFunctions     activationFunction;
          //---
public:
          //--- working with custom activation functions
          void                                  setActivation(const NN_Neurons_Matrix_YX_ActFunctions func = NNNM_YX_Sigmoid) { activationFunction = func; } //set activation function for this layer
          NN_Neurons_Matrix_YX_ActFunctions     getActivation(void) { return activationFunction; } //get the set activation for this layer
          ...
          {         //Print("*** NN_Neurons_Matrix_CLayer::Save **","  m_data_total:",m_data_total);
                    ...
                    {         NN_Neurons_Matrix_YX_CNeuron *n = m_data[i];  //--as Pointer of Cobject   ///NN_Neurons_Matrix_CNeuron driven from Cobject
          ...
          {         //Print("*** NN_Neurons_Matrix_CLayer::Load **","  m_data_total:",m_data_total);
                    ...
                    //---loop and load
                    for(int i = 0; i < m_data_total; i++)
                    {         NN_Neurons_Matrix_YX_CNeuron *n = m_data[i];
