...
//+------------------------------------------------------------------+
enum NN_Neurons_Scaler_ActFunctions
{         NNNS_ReLU,      // f(n)==a== if x < 0 : return 0  else return n
          NNNS_Sigmoid,   // f(n)==a== return  1.0 / (1 + MathPow(2.71828, -n));   ///hardlimit ///semantic hardlimit
          NNNS_SoftMax,
          NNNS_TanH,
          NNNS_CosH,
          NNNS_HyperTan,                //f(n)==a==n
          NNNS_Purelim,                 //f(n)==a==n
          NNNS_HardLim,                  //f(n)==a==n==+-1
          NNNS_SymetricHardLim,          //f(n)==a==n==+1,-1
          NNNS_Saturate,                //f(n)==a==n==0,+1  (0.0-1.0)a=n
          NNNS_SymetricSaturate         //f(n)==a==n==+1,-1 (0.0-1.0)a=n

};
//+------------------------------------------------------------------+
//|                connections container                             |
//+------------------------------------------------------------------+
class NN_Neurons_Scaler_SConnection
{
public:   void   NN_Neurons_Scaler_SConnection() {Totals++;};
          void  ~NN_Neurons_Scaler_SConnection() {};
          double           weight[], deltaWeight[];///vector of one input
          static uint      Totals;
          uint             Total() {return Totals;}
public:
          NN_Neurons_Scaler_SConnection(int idx);
          //--- working with files
          void             Save(int &file_handle);
          void             Load(int &file_handle);

};
uint NN_Neurons_Scaler_SConnection::Totals = 0;
//+------------------------------------------------------------------+
//|             save weights to a file                               |
//+------------------------------------------------------------------+
void NN_Neurons_Scaler_SConnection::Save(int &file_handle)
...
//+------------------------------------------------------------------+
void NN_Neurons_Scaler_SConnection::Load(int &file_handle)
{         if(file_handle == INVALID_HANDLE) return;
          //---
          for(int i = 0; i < ArraySize(weight); i++)
...
//+------------------------------------------------------------------+
NN_Neurons_Scaler_SConnection::NN_Neurons_Scaler_SConnection(int idx)
