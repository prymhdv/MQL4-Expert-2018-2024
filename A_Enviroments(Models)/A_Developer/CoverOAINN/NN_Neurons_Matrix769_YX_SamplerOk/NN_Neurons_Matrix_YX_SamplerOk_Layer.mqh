...
//+------------------------------------------------------------------+
#include<.RTL_Prymhdv\QuartsExperts\A_Enviroments(Models)\A_Developer\CoverOAINN\NN_Neurons_Matrix769_YX_SamplerOk\NN_Neurons_Matrix_YX_SamplerOk_Neuron.mqh>
//+------------------------------------------------------------------+
//|                                                                  |
//|                                                                  |
...
//+------------------------------------------------------------------+
class NNS_Neurons_Matrix_YX_CLayer: public CArrayObj_MQL4 //----array of object make layer
{
public:
          uint                                  iOutputs;
          NNS_Neurons_Matrix_YX_ActFunctions     activationFunction;
          //---
public:
          //--- working with custom activation functions
          void                                  setActivation(const NNS_Neurons_Matrix_YX_ActFunctions func = NNNMS_YX_Sigmoid) { activationFunction = func; } //set activation function for this layer
          NNS_Neurons_Matrix_YX_ActFunctions     getActivation(void) { return activationFunction; } //get the set activation for this layer
          ...
          {         //Print("*** NN_Neurons_Matrix_CLayer::Save **","  m_data_total:",m_data_total);
                    ...
                    {         NNS_Neurons_Matrix_YX_CNeuron *n = m_data[i];  //--as Pointer of Cobject   ///NN_Neurons_Matrix_CNeuron driven from Cobject
          ...
          {         //Print("*** NN_Neurons_Matrix_CLayer::Load **","  m_data_total:",m_data_total);
                    ...
                    //---loop and load
                    for(int i = 0; i < m_data_total; i++)
                    {         NNS_Neurons_Matrix_YX_CNeuron *n = m_data[i];
