...
//+------------------------------------------------------------------+
#include<.RTL_Prymhdv\QuartsExperts\A_Enviroments(Models)\A_Developer\CoverOAINN\NN_Neurons_Matrix769_YX_SamplerOk\NN_Neurons_Matrix_YX_SamplerOk_Base.mqh>
...
//|                                                                  |
//+------------------------------------------------------------------+
class NNS_Neurons_Matrix_YX_CLayer;
class NNS_Neurons_Matrix_YX_CNeuron: public CObject_MQL4
{

public:   //constructors
          NNS_Neurons_Matrix_YX_CNeuron(uint layerIdx, bool IsFristLayer, uint numInputs, uint numOutputs,
                                       uint fm_Index,
                                       NNS_Neurons_Matrix_YX_ActFunctions squashFunc = NNNMS_YX_Sigmoid, double LearningRate = 0.15, double Momentom = 0.5
                                      )//  eta = LearningRate;          alpha = Momentom;
          {         ArrayResize(outputWeights, numOutputs);
                    for(uint i = 0; i < numOutputs; i++) { outputWeights[i] = new NNS_Neurons_Matrix_YX_SConnection(IsFristLayer);}
                    ArrayResize(outputWeightsCalcHiden, numOutputs);
                    for(uint i = 0; i < numOutputs; i++) { outputWeightsCalcHiden[i] = new NNS_Neurons_Matrix_YX_SConnection(IsFristLayer);}
                    ArrayResize(Weights, numInputs);
                    if(numInputs == 0 || IsFristLayer) { ArrayFree(Weights); ArrayResize(Weights, 1);}
                    for(int i = 0; i < ArraySize(Weights); i++)  { Weights[i]       = new NNS_Neurons_Matrix_YX_SConnection(IsFristLayer);}
                    //
                    m_Index = fm_Index;
                    m_layerIdx = layerIdx;
...
public:
          NNS_Neurons_Matrix_YX_ActFunctions     m_Activation;
...
protected:
          NNS_Neurons_Matrix_YX_SConnection        *outputWeights[];  //--each neuron vector weight to other Layer neurons
          NNS_Neurons_Matrix_YX_SConnection        *outputWeightsCalcHiden[];
          NNS_Neurons_Matrix_YX_SConnection        *Weights[];
          double                                  Inputs[];
          double                                  Ourput;
          //---neuron data
          ...
          double           m_Jakobian[];
          //------------------------------------------------------------------------------------------------
          //virtual double   activationFcn(double n);//--'NNS_Neurons_Matrix_YX_CNeuron::activationFcn' - cannot access protected member function         NN_Scaler_Base.mqh  693       56
          ...
          {         double sum = 0.0;
                    ...
                    {         for(int n = 0; n < nextLayer.Total() - 1 /* with out bias */ && !IsStopped(); ++n)
                              {         const NNS_Neurons_Matrix_YX_CNeuron *neuron = nextLayer.At(n);
...
public:   //---neuron data
          ...
          virtual double   activationFcn(double n, double no);               //--'NNS_Neurons_Matrix_YX_CNeuron::activationFcn' - cannot access protected member function         NN_Scaler_Base.mqh  693       56
          ...
          NNS_Neurons_Matrix_YX_SConnection              *getWeigth(int i, int line)const
          ...
          {         //--- A=F(N=WP+B)
                    ...
                    {         NNS_Neurons_Matrix_YX_CNeuron *neuron = prevLayerInput[n];
          ...
          {         for(int n = 0; n < prevLayerInput.Total() - 1 /*with out bias*/ && !IsStopped(); ++n)
                    {         NNS_Neurons_Matrix_YX_CNeuron *prevLayerInputneuron = prevLayerInput.At(n);
          ...
          {         //Print("layerNum:", (string)layerNum, "   NeuronNum:", (string)nidx, "  m_nodeBias:", DoubleToStr(m_nodeBias, 2), "   ArraySize(nidx):", (string)ArraySize(outputWeights)); //
                    ...
                    {         for(int n = 0; n < prevLayerInput.Total() - 1 /*with out bias*/ && !IsStopped(); ++n)
                              {         NNS_Neurons_Matrix_YX_CNeuron *prevLayerInputneuron = prevLayerInput[n];
...
//double NNS_Neurons_Matrix_YX_CNeuron::eta = 0.15;  // Net learning rate
//double NNS_Neurons_Matrix_YX_CNeuron::alpha = 0.5; // Momentum
//+------------------------------------------------------------------+
//|          initiate connection weights save                        |
//+------------------------------------------------------------------+
void NNS_Neurons_Matrix_YX_CNeuron::Save(int &file_handle)
{         //Print("*** NNS_Neurons_Matrix_YX_CNeuron::Save **");
...
//+------------------------------------------------------------------+
void NNS_Neurons_Matrix_YX_CNeuron::Load(int &file_handle)
{         //Print("*** NNS_Neurons_Matrix_YX_CNeuron::Load **");
          if(file_handle == INVALID_HANDLE) return;
          //--- load activation type
          m_Activation   = (NNS_Neurons_Matrix_YX_ActFunctions)FileReadInteger(file_handle);
          //--- load eta and alpha values
          eta   = FileReadDouble(file_handle);
          alpha = FileReadDouble(file_handle); //NNS_Neurons_Matrix_YX_CNeuron::alpha
...
//+------------------------------------------------------------------+
double NNS_Neurons_Matrix_YX_CNeuron::activationFcn(double n, double no = 0.0)
{         switch(m_Activation)
          {         case  NNNMS_YX_ReLU:                    return Activation_reLU(n);              break;
                    case  NNNMS_YX_posline:                 return Activation_posline(n);           break;
                    case  NNNMS_YX_Purelim:                 return Activation_Purelim(n);           break;
                    case  NNNMS_YX_Sigmoid:                 return Activation_sigmoid(n);           break;
                    case  NNNMS_YX_SigmoidSymetric:         return Activation_Sigmoidsymmetric(n);  break;
                    case  NNNMS_YX_SoftMax:                 {double nn[3] = {1, 2, 3 }; Activation_softMax(nn);      return ArrayMinimum(nn);}  break;
                    case  NNNMS_YX_TanH:                    return Activation_tanH(n);              break;
                    case  NNNMS_YX_CosH:                    return Activation_cosH(n);              break;
                    case  NNNMS_YX_HyperTan:                return Activation_HyperTan(n);          break;
                    case  NNNMS_YX_HardLimit:               return Activation_HardLimit(n);         break;
                    case  NNNMS_YX_HardLimitSymetric:       return Activation_HardLimitSymetric(n); break;
                    case  NNNMS_YX_Saturate:                return Activation_Saturate(n);          break;
                    case  NNNMS_YX_SaturateSymetic:         return Activation_SaturateSymetic(n);   break;
                    case  NNNMS_YX_Competive:               return Activation_Competive(n, no);         break;
...
//+------------------------------------------------------------------+
double NNS_Neurons_Matrix_YX_CNeuron::activationDerivative(double n)
{         switch(m_Activation)
          {         case NNNMS_YX_ReLU:             return Derivative_reLU(n);              break;
                    case NNNMS_YX_Purelim:          return Derivative_Purelim(n);           break;
                    case NNNMS_YX_posline:          return Derivative_posline(n);           break;
                    case NNNMS_YX_TanH:             return Derivative_tanH(n);              break;
                    case NNNMS_YX_Sigmoid:          return Derivative_sigmoid(n);           break;
                    case NNNMS_YX_SigmoidSymetric:  return Derivative_Sigmoidsymmetric(n);  break;
                    case NNNMS_YX_Saturate:         return Derivative_Saturate(n);          break;
...
//|           SoftMax activation                                     |
//+------------------------------------------------------------------+
void NNS_Neurons_Matrix_YX_CNeuron::Activation_softMax(double &vector[])///x is vector in cpp or double array
{         //--- range(0.0,1.0)
