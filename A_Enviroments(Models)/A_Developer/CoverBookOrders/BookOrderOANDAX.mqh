...
//#endif
#ifdef __MQL4__
datetime Time(int bar) { return Time[bar]; }
#endif
//#ifdef __MQL5__
...
//#endif
#ifdef __MQL4__
double Close(int bar) { return Close[bar]; }
#endif
// ======================================================================
//  debug.mqh
// ======================================================================
...
//|                                                                  |
//+------------------------------------------------------------------+
void SetDebugMode_(bool on = true)
...
//|                                                                  |
//+------------------------------------------------------------------+
#define LOG_(text) Print(__FILE__,"(",__LINE__,") : ",text)
#define ERROR_CODE_(A) A
#define ERROR_(text, ERROR_CODE_) if (IsDebug_()) Print("Error: ", __FUNCTION__,"(", __LINE__, ") ", text, " ", ERROR_CODE_, " : ", ErrorDescription(ERROR_CODE_))
#define DCHECK_(A) if (IsDebug_() && !(A)) Alert(__FUNCTION__ + "(" + (string)__LINE__ + ")")
#define DCHECK_EQ_(A, B) if (IsDebug_()) DCHECK_(A == B)
#define DEBUG_LOG_(text) if (IsDebug_()) Print(__FILE__,"(",__LINE__,") : ",text)
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
...
//  oanda.mqh
// ======================================================================
const string kProductPath = "QuArtsRoot\\OandaX";
...
{         ResetLastError();
          ...
          {         int file_handle = FileOpen(last_path, FILE_READ | FILE_CSV);
                    ...
                    {         ERROR_("Error in FileOpen, file: " + last_path, GetLastError()); }//
          }
          return NULL; //
}
//+------------------------------------------------------------------+
...
{         string file_path = GetHistoryPath(kOrderBookPath) + kDirSeparator +
          ...
          {         int file_handle = FileOpen(file_path, FILE_READ | FILE_BIN);
                    ...
                    {         // ERROR_("Error in FileOpen, file: " + file_path, GetLastError());
                    }//
          }
          return result;//
}
...
{         Print("-0---BaseHistogram::ConstructData()--");//
          if (!PrepareData())                   {Print("-0---BaseHistogram::ConstructData()--!PrepareData(): ",                          !PrepareData());                        return false;}
          if (cumulative_ && !MakeCumulative()) {Print("-0---BaseHistogram::ConstructData()--cumulative_ && !MakeCumulative(): ",        cumulative_ && !MakeCumulative());      return false;}
          if (delta_ && !MakeDelta())           {Print("-0---BaseHistogram::ConstructData()--delta_ && !MakeDelta(): ",                  delta_ && !MakeDelta());                return false;}
          return true;//
}
//+------------------------------------------------------------------+
//|                                                                  |
...
{         // data should be synchronized
          ...
          if (size != r_size) { Print("----BaseHistogram::MakeDelta--same size: ", (size != r_size)); return false;}
          for (int i = 0; i < size; ++i)
...
{         int size = ArraySize(to);
          ArrayResize(to, size + 1);//inc size
          to[size].price = price;
...
//|                                                                  |
//+------------------------------------------------------------------+
class HistogramUI : public BaseHistogram
{
public:
...
{         Print("----HistogramUI::Draw--!Ready(): ", !Ready() );
          if (true && !Ready())//---provided array data
...
// draw histogram
          ...
          {         double p = low_buys_[i].price;
                    name = "1" + common + "lb" + StringSubstr(DoubleToString(p), 0, 8) + IntegerToString(time);
                    Remember(name);
                    DrawRect(name, wnd, GetTime(x0 - 1), p + step,   GetTime(x0 - 1 - int(width * low_buys_[i].percent / max)), p - step, TP_COLOR, DrawSolid);//
          }
          size = ArraySize(low_sells_);
          for (int i = 0; i < size; ++i)
          {         double p = low_sells_[i].price;
                    name = "1" + common + "ls" + StringSubstr(DoubleToString(p), 0, 8) + IntegerToString(time);
                    Remember(name);
                    DrawRect(name, wnd, GetTime(x0 + 1), p + step, GetTime(x0 + 1 + int(width * low_sells_[i].percent / max)), p - step, SL_COLOR, DrawSolid); //
          }
          size = ArraySize(high_buys_);
          for (int i = 0; i < size; ++i)
          {         double p = high_buys_[i].price;
                    name = "1" + common + "hb" + StringSubstr(DoubleToString(p), 0, 8) + IntegerToString(time);
                    Remember(name);
                    DrawRect(name, wnd, GetTime(x0 - 1), p + step, GetTime(x0 - 1 - int(width * high_buys_[i].percent / max)), p - step, SL_COLOR, DrawSolid);//
          }
          size = ArraySize(high_sells_);
          for (int i = 0; i < size; ++i)
          {         double p = high_sells_[i].price;
                    name = "1" + common + "hs" + StringSubstr(DoubleToString(p), 0, 8) + IntegerToString(time);
                    Remember(name);
                    DrawRect(name, wnd, GetTime(x0 + 1), p + step, GetTime(x0 + 1 + int(width * high_sells_[i].percent / max)), p - step, TP_COLOR, DrawSolid); //
          }
          timestamp_ = time;//
}
//+------------------------------------------------------------------+
...
{
public:
          C_OrderBook(void)     {_OnInit();};
          ~C_OrderBook(void)    {_OnDeinit(REASON_PROGRAM);};
          void  Update()        {_OnTimer();}
          int   _OnInit();
          void  _OnTimer();
          int   _OnCalculate(const int total,
                             const int prev,
                             const datetime & time[],
                             const double & open[],
                             const double & high[],
...
//|                                                                  |
//+------------------------------------------------------------------+
int C_OrderBook:: _OnInit()
{         if (TimeOffset == "AUTO")
          {         if (!GetTimeOffset(OffsetUTC))
                    {         Print("Could not get UTC offset automatically. Please set up the offset manually or run Download Manager during the period when trading is available"); }//
          }
          else
          ...
          Instrument = GenerateInstrumentName( CustomInstrument ? CustomInstrumentName : Symbol(),  CustomInstrument);
          ptr_Histogram = NULL;
          OpenTask();
          Id = GetId();
...
//|                                                                  |
//+------------------------------------------------------------------+
void C_OrderBook:: _OnTimer()
{         OpenTask();
          if (LastPos == 0)   { LastPos = Time(0); }
          if (!ShowLatest)
...
//|                                                                  |
//+------------------------------------------------------------------+
int C_OrderBook:: _OnCalculate(const int total,
                               const int prev,
                               const datetime & time[],
                               const double & open[],
                               const double & high[],
...
//|                                                                  |
//+------------------------------------------------------------------+
void C_OrderBook:: _OnNewBar()
{         _OnTimer(); }
//+------------------------------------------------------------------+
//|                                                                  |
//+------------------------------------------------------------------+
void C_OrderBook:: _OnDeinit(const int reason)
{         FileClose(TaskHandle);
          EventKillTimer();
          delete ptr_Histogram;
          ptr_Histogram = NULL;
...
//+------------------------------------------------------------------+
void DrawHistogram()
{         Print("----DrawHistogram--ShowLatest: ", ShowLatest );
          datetime nearest = 0;
          bool res = false;
          if (ShowLatest)
          ...
          Print("----DrawHistogram--res: ", res );
          if (res)
          {         if (CheckPointer(ptr_Histogram) != POINTER_INVALID)
                    {         if (ptr_Histogram.BaseTime() != nearest + OffsetUTC * 60 * 60 && nearest != 0)
                              {         delete ptr_Histogram;
                                        ptr_Histogram = NULL; } }
                    else
                    {         ptr_Histogram = NULL; }
                    if (NULL == ptr_Histogram)
                    {         OrderBookStruct orderbook;
                              ...
                              {         orderbook.timestamp += OffsetUTC * 60 * 60;
                                        ptr_Histogram = new HistogramUI( orderbook.rate, orderbook.price_points, Cumulative, Difference, ShowOrders, orderbook.timestamp); //
                              } //
                    }//
          }
          else
          {         if (CheckPointer(ptr_Histogram) == POINTER_INVALID)
                    {         PricePoint price_points[];
                              ptr_Histogram = new HistogramUI( (float)Close(0), price_points, Cumulative, Difference, ShowOrders, 0);//
                    } //
          }
          //-------------------------------
          Print("----DrawHistogram--NULL != ptr_Histogram: ", NULL != ptr_Histogram );
          if (NULL != ptr_Histogram)
          {         ptr_Histogram.Draw(0, nearest + OffsetUTC * 60 * 60, 150);//
          } //
}
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
