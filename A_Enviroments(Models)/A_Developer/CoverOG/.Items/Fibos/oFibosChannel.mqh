...
#define Channel_DEF_tooltip "Channel"
//+------------------------------------------------------------------+
//|             Class SoundController     BEGIN                      |
//+------------------------------------------------------------------+
struct S_FIBO_CHANNEL: public _COBF_
{         //============================================================================================================
          void                  S_FIBO_CHANNEL() {TYPE_O_00 = OBJ_FIBOCHANNEL;}; // {Point3XYZ(0, 0, 0, 0, 0, 0);}; //S_HLINE.PointTime.TIME1==0  S_HLINE.PointTime.PRICE1==0  S_HLINE.PointTime.TIME2==0   S_HLINE.PointTime.PRICE2==0
          ...
          bool                  SetRAY(bool ray)                {   RAY                 = ray;          return true;}
          ...
          {         if(IsCreated)return false;
                    ...
                    TYPE_O_00                     /*= FRAME.TYPE*/            = OBJ_FIBOCHANNEL;
          ...
          ////----------------------------------
          //OBJECT.RayRight(RAY); /*ObjectSetInteger(ChartData_ChartInfo.ID, OBJ.CName, OBJPROP_RAY, RAY);*/
          ////----------------------------------
...
class C_FibosChannel: public __COBFA__<S_FIBO_CHANNEL>
{
//===================================================================
private:
          string                        m_name;
...
public:
          void                          C_FibosChannel() {};
          void                          ~C_FibosChannel() {if(Capitan_DeinitErrorCatching)________________________________________________________Xerorr.SuperviserX(_LastError, "-C_FibosChannel Error",  __LINE__, __FUNCTION__); }
//===================================================================CChartObjectFibo
          CChartObjectFiboChannel       GAC;    ///---API  Invoke---///
          S_FIBO_CHANNEL                GDS;    ///---Mine Invoke---///
...
//===================================================================//base on posetive
          ...
          {         GDS.Set_Action_2(WhichMode, IsKey);
                    GDS.Set_Desc_1("Fibos.Channel" + n);
                    ...
                    //---
                    GDS.Set_Common_6(false, 100, true, false, true, Channel_DEF_tooltip + "Fibos.Channel" + n);
                    GDS.Set_Chart_6();
...
//===================================================================
//===================================================================
          bool Create(bool Is_Create, bool Is_Move, const string Channel_Name = "GDS", const string Description = "oChannelDescription",
...
//|                                                                  |
//+------------------------------------------------------------------+
bool C_FibosChannel::Create(bool Is_Create, bool Is_Move, const string Channel_Name = "GDS", const string Description = "oChannelDescription",
                            const bool Background = false, const bool Fill = false, const ENUM_LINE_STYLE style = STYLE_DASH, const int Width = 1,
                            color DescColorIN =  C'36, 52, 69', color DescColorNeg = clrRed, const double Angle = 0,
                            const int FontSize = 7, const string Font = "Arial")
{         ________________________________________________________Xerorr.SuperviserX(_LastError, "-C_Fibo Error",  __LINE__, __FUNCTION__); //|
          ...
          {         LowestBar = iLowest(StSymbol.Current, ChartData_ChartInfo.Timeframe = PERIOD_CURRENT, MODE_LOW, UseCount, UseStart);
                    PriceCordinateTwo = Low[LowestBar];//
          }
          double PriceCordinateMean = Close[(HighestBar + LowestBar) / 2];
          datetime TimeCordinateMean = iTime(StSymbol.Current, ChartData_ChartInfo.Timeframe = PERIOD_CURRENT, MeanBar); // Time[50];
          TimeCordinateMean = TimeCordinateTwo;
          PriceCordinateMean = PriceCordinateOne;
          //===========================
          if(ObjectFind(ChartData_ChartInfo.ID, Channel_Name) >= 0 && Is_Move )
          {         int CordinateXone = 0; int  CordinateYone = 0;
                    ...
                    ObjectSetInteger(0, Channel_Name, OBJPROP_XDISTANCE, CordinateXone);//Label_.X_Distance(XCordinate);
                    ObjectSetInteger(0, Channel_Name, OBJPROP_YDISTANCE, CordinateYone);////Label_.Y_Distance(YCordinate); //
                    return true;//
          }
          if(ObjectFind(ChartData_ChartInfo.ID, Channel_Name) >= 0 && !Is_Create) {return ObjectDelete(ChartData_ChartInfo.ID, Channel_Name);}
          if(ObjectFind(ChartData_ChartInfo.ID, Channel_Name) < 0)
                    if(! GAC.Create(ChartData_ChartInfo.ID, Channel_Name, ChartData_ChartInfo.SubWindow, TimeCordinateOne, PriceCordinateOne, TimeCordinateTwo, PriceCordinateTwo, TimeCordinateMean, PriceCordinateMean))Alert("-Line:", __LINE__);;
          if(ObjectFind(ChartData_ChartInfo.ID, Channel_Name) >= 0)
          {         //
                    GAC.RayLeft();
                    //
                    ...
                    ________________________________________________________Xerorr.SuperviserX(_LastError, "-C_Fibo Error",  __LINE__, __FUNCTION__); //
                    //============================//============================
                    Dev_Object.FiboService.FiboLeveler(GAC, Channel_Name, Width, style, DescColorIN, DescColorNeg, Gradient) ; //
                    return true;//
          } ///////////////////////////////////////////
          ________________________________________________________Xerorr.SuperviserX(_LastError, "-C_Fibo Error",  __LINE__, __FUNCTION__); //|
          return false;//
