...
#define TimeZone_DEF_tooltip "TimeZone"
//+------------------------------------------------------------------+
//|             Class SoundController     BEGIN                      |
//+------------------------------------------------------------------+
struct S_FIBO_TIMESZONE: public _COBF_
{         //============================================================================================================
          void                  S_FIBO_TIMESZONE() {TYPE_O_00 = OBJ_FIBOTIMES;}; // {Point2XY(0, 0, 0, 0);}; //S_HLINE.PointTime.TIME1==0  S_HLINE.PointTime.PRICE1==0  S_HLINE.PointTime.TIME2==0   S_HLINE.PointTime.PRICE2==0
          ...
          {         if(IsCreated)return false;
                    ...
                    TYPE_O_00                     /*= FRAME.TYPE*/            = OBJ_FIBOTIMES;
...
class C_FibosTimeZone: public __COBFA__<S_FIBO_TIMESZONE>  
{
//===================================================================
private:
          string                        m_name;
...
//===================================================================
public:
          void                          C_FibosTimeZone() {};
          void                          ~C_FibosTimeZone() {if(Capitan_DeinitErrorCatching)________________________________________________________Xerorr.SuperviserX(_LastError, "-C_FibosTimeZone Error",  __LINE__, __FUNCTION__); }
//===================================================================CChartObjectFibo
          CChartObjectFiboTimes         GAC;
          S_FIBO_TIMESZONE              GDS;
...
//===================================================================
          ...
          {         GDS.Set_Action_2(WhichMode, IsKey);
                    GDS.Set_Desc_1("Fibos.TimeZone" + n);
                    ...
                    //---
                    GDS.Set_Common_6(false, 100, true, false, true, TimeZone_DEF_tooltip + "Fibos.TimeZone" + n);
                    GDS.Set_Chart_6();
...
//===================================================================
          bool Create(bool Is_Create, bool Is_Move, const string TimesZone_Name = "UseFiboTimes", const string Description = "UseFiboTimesDescription",
...
//|                                                                  |
//+------------------------------------------------------------------+
bool C_FibosTimeZone::Create(bool Is_Create, bool Is_Move, const string TimesZone_Name = "UseFiboTimes", const string Description = "UseFiboTimesDescription",
                             const bool Background = false, const bool Fill = false, const ENUM_LINE_STYLE style = STYLE_DASH, const int Width = 1,
                             color DescColorIN =  C'36, 52, 69', color DescColorNeg = clrRed,
                             const double Angle = 0, const int FontSize = 7, const string Font = "Arial")
...
//===========================
          if(ObjectFind(ChartData_ChartInfo.ID, TimesZone_Name) >= 0 && Is_Move )
          {         int CordinateXone = 0; int  CordinateYone = 0;
                    ...
                    ObjectSetInteger(0, TimesZone_Name, OBJPROP_XDISTANCE, CordinateXone);//Label_.X_Distance(XCordinate);
                    ObjectSetInteger(0, TimesZone_Name, OBJPROP_YDISTANCE, CordinateYone);////Label_.Y_Distance(YCordinate); //
                    return true;//
          }
          if(ObjectFind(ChartData_ChartInfo.ID, TimesZone_Name) >= 0 && !Is_Create) {return ObjectDelete(ChartData_ChartInfo.ID, TimesZone_Name);}
          if(ObjectFind(ChartData_ChartInfo.ID, TimesZone_Name) < 0)
                    if(! GAC.Create(ChartData_ChartInfo.ID, TimesZone_Name, ChartData_ChartInfo.SubWindow, TimeCordinateOne, PriceCordinateOne = Bid, TimeCordinateTwo, PriceCordinateTwo = Bid))
                              ________________________________________________________Xerorr.SuperviserX(_LastError, "C_Fibo Error",  __LINE__, __FUNCTION__); //|
          if(ObjectFind(ChartData_ChartInfo.ID, TimesZone_Name) >= 0)
          {         GAC.Color(DescColorIN);
                    ...
                    Dev_Object.FiboService.FiboLeveler(GAC, TimesZone_Name, Width, style, DescColorIN, DescColorNeg, Gradient) ; //
                    return true;//
          }
          ________________________________________________________Xerorr.SuperviserX(_LastError, "-C_Fibo Error",  __LINE__, __FUNCTION__); //|
          return false;//
