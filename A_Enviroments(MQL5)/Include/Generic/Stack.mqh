...
//|                                             https://www.mql5.com |
//+------------------------------------------------------------------+
#include <.RTL_Prymhdv\QuartsExperts\A_Enviroments(MQL5)\Include\Generic\Interfaces\ICollection.mqh>
#include <.RTL_Prymhdv\QuartsExperts\A_Enviroments(MQL5)\Include\Generic\Internal\ArrayFunction.mqh>
#include <.RTL_Prymhdv\QuartsExperts\A_Enviroments(MQL5)\Include\Generic\Internal\EqualFunction.mqh>
//+------------------------------------------------------------------+
//| Class CStack<T>.                                                 |
...
public:
          ...
          bool              Add(T value);               //Inserts an value at the top
          bool              Push(T value);              //Inserts an values at the top
          ...
          bool              Remove(T item);
          //--- methods of access to protected data
          T                 Peek(void);                 //Returns the value at the top of the CStack<T> without removing.
          T                 Pop(void);                  //Removes and returns the value at the top
          //--- methods of test
          void Main(bool b)
          {         if(!b)   { return; }
                    //----------------
                    ...
                    stack_int.Push(40);
                    //
                    Alert("frist stack_int.Peek: ", stack_int.Peek());
                    stack_int.Pop();//Removes and returns the value at the top
                    Alert("after pop stack_int.Peek: ", stack_int.Peek());
                    stack_int.Push(20);
                    Alert("after push 20 stack_int.Peek: ", stack_int.Peek());
                    stack_int.Remove(30);
                    Alert(" after remove 30 stack_int.Peek: ", stack_int.Peek());
                    stack_int.Clear();
                    //
          } //
};
...
{         int size = ArraySize(m_array);
          ...
          if(m_size == size)
          {         //--- increase capacity
                    if(size == 0)   ArrayResize(m_array, m_default_capacity);
                    else            ArrayResize(m_array, 2 * size); //
          }
