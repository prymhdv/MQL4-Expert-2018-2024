...
public:
   ...
   double            Get(const int i) { return m_array[i];  };        
   //--- overloading
...
public:
   //--- constructors, destructor
                     ...
                     CMatrixDouble(const int rows,const int cols,double &Data[]);
                    ~CMatrixDouble(void);
   //--- methods
   int               Size(void) const;
   ...
   {       int rowsNum = ArraySize(m_rows);
           int colsNum = m_rows[0].Size();
           ArrayResize(Data,rowsNum*colsNum);//
   for(int r=0;r<rowsNum;r++)
        {for(int c=0;c< colsNum;c++)
           { //Print("getArray Dim:  (",rowsNum,",",colsNum,"):(",r,",",c,")");
            int index =(r*rowsNum)+c;
            Data[index]=m_rows[r].Get(c);//
           }//
...
//| Constructor with two parameters                                  |
//+------------------------------------------------------------------+
CMatrixDouble::CMatrixDouble(const int rows,const int cols,double &Data[])
  {
   ArrayResizeAL(m_rows,rows);
   for(int i=0;i<rows;i++)
      m_rows[i].Resize(cols);
