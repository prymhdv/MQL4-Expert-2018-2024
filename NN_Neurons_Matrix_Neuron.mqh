...
//+------------------------------------------------------------------+
#include<.RTL_Prymhdv\QuartsExperts\A_Enviroments(Models)\A_Developer\CoverOAINN\NN_Neurons_Matrix\NN_Neurons_Matrix_Base.mqh>
//+------------------------------------------------------------------+
//|                                                                  |
//|                                                                  |
//|                  Neuron Object                                   |
...
protected:
          ...
          uint             m_myIndex;
          double           m_gradient;
          //double           m_nodeBias;  //--neuron not has inside bias
          ...
          {         double sum = 0.0;
                    ...
                    {         const NN_Neurons_Matrix_CNeuron *neuron = nextLayer.At(n); sum += outputWeights[n].weight * neuron.m_gradient; } return sum;//
          }
...
public:   //constructors
          ...
          //void             setNodeBias(double  val)     { m_nodeBias = val; } //--neuron not has inside bias
          ...
          {         double sum = 0.0;
                    ...
                    {         NN_Neurons_Matrix_CNeuron *neuron = prevLayer.At(n);
                    ...
                    {         //this is layer bias neuron!
                              ...
                              NN_Neurons_Matrix_CNeuron *neuronBias = prevLayer.At(prevLayer.Total() - 1/*this is bias*/);
                              m_nodeVal = activationFcn(/*bias*/neuronBias.getNodeVal() + sum, 0.0); //+ getNodeBias()
                    } //
          }
          ...
          void             updateInputWeights(Layer &prevLayer, double f_recentAverageError)
          {         for(int n = 0; n < prevLayer.Total()/*with bias*/ && !IsStopped(); ++n)
                    {         NN_Neurons_Matrix_CNeuron *neuron = prevLayer.At(n);
                              ...
                              //Print("oldDelta: ", (string)oldDelta, "  newDelta: ", (string)newDelta, "  neuron.getNodeVal(): ", (string)neuron.getNodeVal()); Sleep(100); //
                              //newDelta = (eta * neuron.getNodeVal() * m_gradient) + (alpha * (f_recentAverageError / (oldDelta + 1))) + (0) ; /**not zero diving*/
                              //newDelta = (eta * neuron.getNodeVal() * m_gradient) + (alpha * f_recentAverageError * oldDelta ) + (0) ;
